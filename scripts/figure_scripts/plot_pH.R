...
pH_sigletters <- read_csv("~/CAPSTONE_PUBLICATION/data/analyzed_data/drivers_analyzed_data/significance_letters/eal_pH_sigletters.csv")
spec(pH_sigletters)
...
boxplot <- ggplot(pH_sigletters, aes(x = MonitoringLocationName, y = pH, fill = MonitoringLocationName)) +
  ...
  geom_text(data = letter_positions,
            aes(x = MonitoringLocationName, y = pH + 0.05, label = sig_letter),
...
# Export plot as PNG
ggsave("~/CAPSTONE_PUBLICATION/figures/driver_figures/eal_pH_boxplot.png", plot = boxplot, width = 8, height = 6, dpi = 300)
